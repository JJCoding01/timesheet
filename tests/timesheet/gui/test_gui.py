...
def test_default_values(app):
    ...
    assert (
        app.labelThisWeekGoals.text()
        == "Goals for this week (Dec 26 to Jan 01)"
    )
    assert (
        app.labelFutureGoals.text() == "Goals for next week (Jan 02 to Jan 08)"
    )
...
def test_this_week_goal_labels(app):
    ...
    assert (
        app.labelThisWeekGoals.text()
        == "Goals for this week (May 17 to May 23)"
    )
...
def test_future_week_goal_labels(app):
    ...
    assert app.date.text() == "May 23 2020"
    assert (
        app.labelFutureGoals.text() == "Goals for next week (May 24 to May 30)"
    )
